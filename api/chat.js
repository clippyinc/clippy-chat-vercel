export default async function handler(req,res){
  ...
  try{
    ...
    const groqKey=(process.env.GROQ_API_KEY||"").trim();const openaiKey=(process.env.OPENAI_API_KEY||"").trim();const tavilyKey=(process.env.TAVILY_API_KEY||"").trim();const supabaseUrl=(process.env.SUPABASE_URL||"").trim();const supabaseKey=(process.env.SUPABASE_ANON_KEY||process.env.SUPABASE_SERVICE_ROLE_KEY||"").trim();
    ...
    const lastUserMessage=[...cleanHistory].reverse().find(m=>m.role==="user");const lastUserQ=lastUserMessage?.content?.trim()||"";if(!lastUserQ)return res.status(400).json({error:"No user message"});
    const now=new Date();const localDateTime=now.toLocaleString("en-PH",{timeZone:"Asia/Manila",dateStyle:"full",timeStyle:"long"});
    ...
    const supabaseGet=(t,q)=>supabaseRequest("GET",t,q);const supabaseInsert=(t,p)=>supabaseRequest("POST",t,"",p);
    ...
    if(groqKey){
      ...
      for(const model of models){
        try{
          ...
          const d=await r.json();if(r.ok&&d.choices?.[0]?.message?.content){aiResult=d.choices[0].message.content;break;}lastErr=d?.error?.message||`Groq ${r.status}`;
    ...
    if(!aiResult&&openaiKey){
      try{
        ...
        const d=await r.json();if(r.ok&&d.choices?.[0]?.message?.content)aiResult=d.choices[0].message.content;else lastErr=d?.error?.message;
      }catch(e){lastErr=e.message;}
    }
    ...
    let reply=(parsed.reply||"Yep bud.").trim();let actions=Array.isArray(parsed.actions)?parsed.actions:[];
