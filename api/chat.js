export default async function handler(req, res) {
  ...
  try{
    ...
    if(!geminiKey&&!groqKey) return res.status(200).json({reply:"1. Summary: No API key\n2. Fix: Add GEMINI_API_KEY (AQ... key mo) + GROQ_API_KEY sa Vercel > Settings > Env Vars\n3. Get Gemini: aistudio.google.com"});

    // TOKEN KILLER FIXED - 8 msgs x 500 chars
    ...
    async function supabaseRequest(method,table,query="",body=null){if(!supabaseUrl||!supabaseKey)return {ok:false,data:null};try{const r=await fetch(`${supabaseUrl}/rest/v1/${table}${query}`,{method,headers:supabaseHeaders(method!=="GET"),body:body?JSON.stringify(body):undefined});const text=await r.text();let data=null;try{data=text?JSON.parse(text):null;}catch{data=text;}return {ok:r.ok,data};}catch(e){return {ok:false,data:null};}}
    ...
    if(use_db&&supabaseUrl&&supabaseKey){
      try{
        ...
        if(memoryRes.ok&&memoryRes.data?.length){const qWords=lastUserQ.toLowerCase().split(/\s+/).filter(w=>w.length>=3);const scored=memoryRes.data.map(m=>{let sc=0;const c=String(m.content||'').toLowerCase();qWords.forEach(w=>{if(c.includes(w))sc+=2;});return {...m,_score:sc};}).sort((a,b)=>b._score-a._score).slice(0,4);contextData.memories="\nMEMORIES:\n"+scored.map(m=>`[${m.role}] ${m.content}`).join("\n").slice(0,600);}
      }catch{}
    }

    let webContext="";let webImages=[];
    if(tavilyKey && /weather|news|latest|today|price|search|who is|what is|nba|score|stock|usd|php/i.test(lastUserQ)){
      try{
    ...
    // DETAILED OUTLINE FORMAT - MANDATORY
    const personalities={
...
RULES: If file/image attached: extract numbers. If missing data: ASK, don't hallucinate. Bullets > essay.`,
      work:`You are Clippy, work mode - store analyst for Happy's Place. Professional barkada.
FORMAT:
1. Summary: ADS, Target, % vs Target
2. Breakdown: 2.1 Total, 2.2 In-Store, 2.3 Delivery + formula
3. Status + Next Action
Ask if missing actual ADS, don't guess. Boss/bossing 30% only.`
    };
...
${webContext?`WEB:${webContext}`:""}
${webImages.length?`\nIMAGES:\n${webImages.map(u=>`![image](${u})`).join("\n")}`:""}
Respond JSON ONLY: {"reply":"outline formatted reply","actions":[]}`;

    let finalMessages=[{role:"system",content:systemPrompt},...cleanHistory];

    // Handle images for Gemini vision
    let useGeminiVision = images.length>0 && geminiKey;

    let aiResult=null,lastErr="";

    // 1. PRIMARY: GEMINI (AQ... or AIzaSy... both work!)
    if(geminiKey){
      try{
        console.log("Trying Gemini primary with AQ key");
        const contents=[
          {role:"user",parts:[{text:systemPrompt}]},
          {role:"model",parts:[{text:"Gets bud! Outline + numbers mode ready!"}]}
        ];
        ...
        if(r.ok&&d.candidates?.[0]?.content?.parts?.[0]?.text){
          ...
          console.log("Gemini success with AQ key");
        } else {
    ...
    // 2. BACKUP: GROQ (2 working models)
    if(!aiResult && groqKey){
    ...
    if(!aiResult) return res.status(200).json({reply:`1. Summary: AI failed\n2. Error: ${lastErr}\n3. Fix: Check Vercel env GEMINI_API_KEY (AQ...) + GROQ_API_KEY`,error:lastErr});

    let parsed;
    try{parsed=JSON.parse(aiResult);}catch(e){parsed={reply:String(aiResult).slice(0,2000),actions:[]};}
    ...
    if(use_db&&supabaseUrl&&supabaseKey&&actions.length){
      for(const a of actions){
        ...
        try{
          ...
          if(a.type==="schedule"&&a.data.title)await supabaseInsert("schedules",{business_id:businessId,title:String(a.data.title).slice(0,400),scheduled_at:a.data.scheduled_at||new Date().toISOString(),status:a.data.status||"pending"});
        }catch{}
      }
    }
    return res.status(200).json({reply:reply.replace(/```[\s\S]*?```/g,"").trim(),mode,use_db,images:webImages,provider:geminiKey&&aiResult?"gemini":"groq"});
  }catch(e){console.error("FATAL",e);return res.status(500).json({reply:`1. Server Error: ${e.message}\n2. Fix: Check Vercel logs`,error:e.message});}
}
